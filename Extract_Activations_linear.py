...
test_TL = np.reshape(input_normalized_transfer[0:200:10,:,:,:],(20,128,128,2))
...
for k in [10]:
    for j in combs:
        ...
        model.load_weights('./Weights/weights_TL_from_Re_1k_to_Re_100k_per_train_'+str(k)+'_layers'+layers_str)
        for i in range(11):
            inp = model.inputs
            ...
            layer_activations['activations_TL'+str(i+1)] = np.squeeze(inter_output(test_TL))

            layer.activation = None
            model.compile()

            layer_activations['activations_linear'+str(i+1)] = np.squeeze(inter_output(test_TL))
            model = build_model(**params)
            model.load_weights('./weights_TL_from_Re_1k_to_Re_100k_per_train_'+str(k)+'_layers'+layers_str)
        savemat('./activations_TL_from_Re_1k_to_Re_100k_per_train_' + str(k) + '_layers' + layers_str + '_linear.mat',layer_activations)
