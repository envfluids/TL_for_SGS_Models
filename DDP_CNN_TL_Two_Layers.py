...
combs = list(combinations(range(1,12),2))
print(len(combs))
...
for j in combs:
    ...
    model.save_weights('./weights_TL_from_Re_1k_to_Re_100k_per_train_'+percent+'_layers'+layers_str)
    model.save('./Models/CNN_TL_from_Re_1k_to_Re_100k_per_train_'+percent+'_layers'+layers_str)
    prediction=model.predict(input_normalized_val[0:100,:,:,:])


    savemat('./prediction_TL_from_Re_1k_to_Re_100k_per_train_'+percent+'_layers'+layers_str+'.mat',dict([('test',output_normalized_val[:100,:,:,:]),('input',input_normalized_val[0:100,:,:,:]),('prediction',prediction[0:100,:,:])]))
